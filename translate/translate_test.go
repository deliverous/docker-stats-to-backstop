...
func Test_ParseJson_NetworkStats_Rx(t *testing.T) {
	stats, _ := ParseJson(jsonContainerStats)
	if stats.Network.RxBytes != 120062 {
		t.Errorf("rx bytes failed: got %d", stats.Network.RxBytes)
	}

	if stats.Network.RxPackets != 1425 {
		t.Errorf("rx packets failed: got %d", stats.Network.RxPackets)
	}

	if stats.Network.RxErrors != 1 {
		t.Errorf("rx errors failed: got %d", stats.Network.RxErrors)
	}

	if stats.Network.RxDropped != 2 {
		t.Errorf("rx dropped failed: got %d", stats.Network.RxDropped)
	}
}
...
const jsonContainerStats = `
  {
    ...
    "network":{"rx_bytes":120062,"rx_packets":1425,"rx_errors":1,"rx_dropped":2,"tx_bytes":160652,"tx_packets":1257,"tx_errors":0,"tx_dropped":0},
